...
export default function WireTransaction() {
  ...
  const renderItem = ({ item, index }) => (
    <View style={styles.row}>
      ...
      <Text style={[styles.cell, { minWidth: 120 }]}>{item.createdAt}</Text>
      <Text style={[styles.cell, { minWidth: 100 }]}>{item.timeCreated}</Text>
      <Text style={[styles.cell, { minWidth: 100 }]}>{item. wire_status}</Text>
    </View>
  );
  return (
    ...
    <ScrollView horizontal>
      <View style={[styles.table, { minWidth: screenWidth }]}>
        <Text style={{ fontSize: 20, width: 150, marginBottom: 15 }}>Wire Transfer Transactions</Text>
