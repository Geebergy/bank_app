...
export default function DomesticTransactions() {
  ...
  const fetchTransactions = async () => {
    ...
    await axios.get(`https://bank-app-4f6l.onrender.com/user/getDomesticByUserId/${userId}`)
      .then(response => {
  ...
  const renderItem = ({ item, index }) => (
    <View style={styles.row}>
      ...
      <Text style={[styles.cell, { minWidth: 80 }]}>{item.acct_type}</Text>
      <Text style={[styles.cell, { minWidth: 150 }]}>{item.acct_name}</Text>
      <Text style={[styles.cell, { minWidth: 200 }]}>{item.acct_remarks}</Text>
      <Text style={[styles.cell, { minWidth: 120 }]}>{item.created_at}</Text>
      <Text style={[styles.cell, { minWidth: 100 }]}>{item.timeCreated}</Text>
      <Text style={[styles.cell, { minWidth: 100 }]}>{item.dom_status}</Text>
    </View>
  );
  return (
    ...
    <ScrollView horizontal>
      <View style={[styles.table, { minWidth: screenWidth }]}>
        <Text style={{ fontSize: 20, width: 150, marginBottom: 15 }}>Domestic Transfer Transactions</Text>
