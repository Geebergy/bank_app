...
export default function CreditDebitTransactions() {
  ...
  const fetchTransactions = async () => {
    ...
    await axios.get(`https://bank-app-4f6l.onrender.com/user/getDeposits/${userId}`)
      .then(response => {
  ...
  const renderItem = ({ item, index }) => (
    <View style={styles.row}>
      ...
      <Text style={[styles.cell, { minWidth: 100 }]}>{item.crypto_status}</Text>
    </View>
  );
  return (
    ...
    <ScrollView horizontal>
      <View style={[styles.table, { minWidth: screenWidth }]}>
        <Text style={{ fontSize: 20, width: 150, marginBottom: 15 }}>Credit/Debit Transactions</Text>
